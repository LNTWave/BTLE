...
// bluetoothle.writeQ(writeSuccess, writeError, paramsObj[j]);
    ...
    sendCompleteDataToBLE();
    // Do it....
//    WriteBluetoothDeviceEx();
    
}

function sendCompleteDataToBLE() {			
	var u8Sub  = u8TxBuff.subarray(0, uTxMsgLen);
	var u64    = bluetoothle.bytesToEncodedString(u8Sub);	
	
	var paramsObj = [];
	if( (window.device.platform == iOSPlatform) &&  (swVerBtScan.localeCompare("01.00") == 0) ) {
		paramsObj[0] = {"address":btAddr, "value":u64, "serviceUuid":bridgeServiceUuid, "characteristicUuid":bridgeRxCharacteristicUuid};      
	} else {
		paramsObj[0] = {"address":btAddr, "value":u64, "serviceUuid":bridgeServiceUuid, "characteristicUuid":bridgeRxCharacteristicUuid, "type":"noResponse"};
	}    
	bluetoothle.writeQ(writeSuccess, writeError, paramsObj[0]);
	isSouthBoundIfCnx = false;
}
    ...
    for( j = 0; j < maxPhoneBuffer; j++ )
    {
        ...
        if( uTxBuffIdx < uTxMsgLen )
        {
            ...
            if( (window.device.platform == iOSPlatform) &&  (swVerBtScan.localeCompare("01.00") == 0) )
            {
                ...
                paramsObj[j] = {"address":btAddr, "value":u64, "serviceUuid":bridgeServiceUuid, "characteristicUuid":bridgeRxCharacteristicUuid};
            ...
            else
            {
                ...
                paramsObj[j] = {"address":btAddr, "value":u64, "serviceUuid":bridgeServiceUuid, "characteristicUuid":bridgeRxCharacteristicUuid, "type":"noResponse"};
            }
