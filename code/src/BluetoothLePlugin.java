...
public class BluetoothLePlugin extends CordovaPlugin
{
  ...
  /* Added by L&T */
  private final String writeQActionName = "writeQ";
...
//Actions
  ...
  public boolean execute(String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException
  {
    ...
    else if (writeQActionName.equals(action))
    {
      cordova.getThreadPool().execute(new Runnable() {
        public void run() {
        	splitDataBytes(args, callbackContext);
        }
      });
      return true;
    }
  ...
  /* Added by L&T */
	public void splitDataBytes(JSONArray args, CallbackContext callbackContext)  {
		...
		if (value.length > maxDataSize) {
			...
			int maxPackets = value.length / maxDataSize;
			
			int remainder = value.length % maxDataSize;

			if (remainder > 0)
				maxPackets = maxPackets + 1;			

			byte[][] queuedValue = new byte[maxPackets][maxDataSize];

			int dataStart = 0;

			for (int i = 0; i < maxPackets; i++) {				

				if (remainder > 0 && (i + 1) == maxPackets) {

					maxDataSize = remainder;
				}

				queuedValue[i] = new byte[maxDataSize];

				for (int j = 0; j < maxDataSize; j++) {

					if ((dataStart) > (value.length - 1)) {
						break;
					}

					queuedValue[i][j] = value[dataStart];					

					dataStart++;
				}
				writeQAction(queuedValue[i]);
			}
		} else {
			writeQAction(value);
		}
	...
	private void writeQAction(byte[] value)
	{       
	
	  String address = getAddress(jsonArgs);
	  ...
	  BluetoothGattService service = getService(bluetoothGatt, jsonArgs);
	  ...
	  BluetoothGattCharacteristic characteristic = getCharacteristic(jsonArgs, service);
...
//	  byte[] value = getPropertyBytes(this.jsonArgs, keyValue);
	  ...
	  int writeType = this.getWriteType(jsonArgs);
	  characteristic.setWriteType(writeType);
 ...
 /* private List<String> getActualData (JSONObject obj, String key) {
